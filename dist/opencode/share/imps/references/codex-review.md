...
`/imps` calls `scripts/run-code-review.sh` after the merged diff passes its
...
runtime from Claude Code's own `~/.config/opencode/plugins/installed_plugins.json` — the install
...
| `IMPS_CLAUDE_PLUGINS_MANIFEST` | `~/.config/opencode/plugins/installed_plugins.json` | Install manifest to query |
