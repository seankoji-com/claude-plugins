---
description: >
  Use when an existing /imps pull request should be monitored and repaired for review
...
```bash
ls ~/.config/opencode/imps/runs/*.prs.json 2>/dev/null || true
```
...
dispatched run at the `standard` tier, resolved to a model id and passed as `opencode run -m <id>` at
invocation, never in frontmatter (matrix Item 3 measured that OpenCode does not honor Claude Code's `model:` frontmatter convention).
