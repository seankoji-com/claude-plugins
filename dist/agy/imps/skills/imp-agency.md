...
- `--out <path>` (optional) — where to write the plan. Default:
  `~/.gemini/config/imps/audits/<repo-name>-<YYYY-MM-DD>.md`. Must resolve to an
...
OUT="${out:-}"
[ -n "$OUT" ] || OUT=~/.gemini/config/imps/audits/"$(basename "$REPO_ROOT")-$(date +%F).md"
...
Do not launch `/imps` yourself, and do not start fixing findings — the operator
