...
`/imps 42 43` scouts those issues and drives them through implementation and gates to
an operator handoff (add `--personas` to also run the persona-review panel).

**Persona panel is opt-in (`PERSONA_PANEL`).** `/imps` parses `--personas` and hands
...
`[imps-status]` marker — `/prs`'s comment filter (see `commands/prs.md`) skips any
