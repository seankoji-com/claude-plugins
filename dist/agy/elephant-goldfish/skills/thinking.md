...
- `~/.gemini/config/elephant-goldfish/learnings.md` — cross-project rules
- `.agy/elephant-goldfish/learnings.md` — rules for this repo

Then resolve state in **one call** rather than listing and reading files:

```bash
python3 "__PLUGIN_ROOT__/scripts/thinking_state.py" list
```
...
```bash
python3 "__PLUGIN_ROOT__/scripts/thinking_state.py" init <slug> \
...
```bash
python3 "__PLUGIN_ROOT__/scripts/gh_publish.py" ensure <slug> --mode <issue|discussion>
```
...
- `__PLUGIN_ROOT__/templates/probes.research.md`
- `__PLUGIN_ROOT__/templates/probes.implementation.md`
...
- For `implementation`, ground questions in the repo. Delegate mechanical recon — where does
  ...
  a cheap read-only `scout` if one is defined, otherwise `Explore`, otherwise just read the files
...
Synthesise, don't transcribe. Follow `__PLUGIN_ROOT__/templates/discovery.skeleton.md`
...
```bash
python3 "__PLUGIN_ROOT__/scripts/gh_publish.py" post <slug> discovery.md
```
...
Follow `__PLUGIN_ROOT__/templates/spec.skeleton.md`. Then:

```bash
python3 "__PLUGIN_ROOT__/scripts/gh_publish.py" post <slug> spec.md
```
...
```bash
python3 "__PLUGIN_ROOT__/scripts/thinking_state.py" gate <slug> --require discovery,spec \
  && python3 "__PLUGIN_ROOT__/scripts/render_handoff.py" <slug> \
  && python3 "__PLUGIN_ROOT__/scripts/gh_publish.py" post <slug> handoff.md
```
...
bash "__PLUGIN_ROOT__/scripts/audit-log.sh" --plugin elephant-goldfish \
  --command /thinking --exit-status "${AUDIT_STATUS:-completed}" \
