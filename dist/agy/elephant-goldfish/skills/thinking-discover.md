...
GitHub publishing — that is a reference to the other platform, so it keeps its Claude Code
name. The scripted equivalent here is the `/thinking` skill in this same plugin; this skill is
the portable path: same process, no scripts required.
...
- Work the decision frontier: ask only decisions whose prerequisites are settled. Discoverable
  facts are the agent's job; resolve them from disk, tools, or primary sources. When an answer
