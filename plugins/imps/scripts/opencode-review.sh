...
CONFIG_PATH="${IMPS_OPENCODE_CONFIG_PATH:-$HOME/.config/opencode/opencode.json}"
MODEL="${IMPS_OPENCODE_REVIEW_MODEL:-litellm/deepseek-v4-flash}"
VARIANT="${IMPS_OPENCODE_REVIEW_VARIANT:-}"
REPO=""
BASE=""
HEAD="HEAD"
GOAL=""
...
case "$MODEL" in
  litellm/deepseek-v4-flash) PROVIDER="litellm" ;;
  *) fail model_rejected "code review is pinned to litellm/deepseek-v4-flash" ;;
esac
case "$VARIANT" in
...
[ -f "$CONFIG_PATH" ] || fail provider_config_missing "OpenCode LiteLLM provider configuration is unavailable"
jq -e '.provider.litellm | type == "object"' "$CONFIG_PATH" >/dev/null 2>&1 || fail provider_config_missing "OpenCode LiteLLM provider configuration is unavailable"
...
# Query in a temporary XDG tree too. Only the pinned LiteLLM provider configuration is
# copied; unrelated providers and the user's remaining OpenCode configuration stay out.
...
}' "$CONFIG_PATH" > "$TMP_ROOT/config/opencode/opencode.json" || fail provider_config_copy_failed 'cannot isolate LiteLLM provider configuration'
chmod 600 "$TMP_ROOT/config/opencode/opencode.json" || fail provider_config_copy_failed 'cannot protect temporary provider configuration'
...
if ! jq -e --arg model "$MODEL" '.. | strings | select(. == $model)' "$TMP_ROOT/models.json" >/dev/null 2>&1 \
  && ! grep -Fqx -- "$MODEL" "$TMP_ROOT/models.json"; then
  fail model_unavailable 'configured model is not reported by opencode'
fi
...
cp "$PLUGIN_ROOT/references/code-review-brief.md" "$SNAPSHOT/code-review-brief.md" || fail snapshot_failed 'cannot capture review brief'
...
repository standards, GOAL.md, and the cross-lineage code review brief.

Apply code-review-brief.md's architecture, line-correctness, and contract-fit checks. Read the full
