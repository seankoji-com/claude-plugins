...
_UNEXPANDED_PLACEHOLDER = re.compile(
    r"^\$\{(?:user_config\.[A-Za-z0-9_]+|CLAUDE_[A-Z_]+)\}\Z"
)
...
def save_quota_state(state):
    ...
    try:
        ...
        raise SidecarError(
            f"Cannot persist quota state: {e}. "
            f"Check disk space and file permissions at {path}."
        )
