<!-- REPLACE-SECTION: ## Phase 0 — Preflight + fingerprint (you; no subagents, no Workflow yet) -->
...
<!-- REPLACE-SECTION: ## Phase 1 — Sync the Workflow script -->
## Phase 1 — Run the expedition yourself

There is no `Workflow` tool here, so there is no background script to sync and no
`scriptPath` to pass — the expedition is control flow you execute in this session, in the
order below. Run the phases **serially**; the sequencing is the whole point: dedupe before
ranking, cap before cloning, retry cloning once, and do not start synthesis until every
analysis has returned.

**Models.** No per-skill model field is emitted: no matrix item establishes one for this
platform, so none is assumed. Claude's `haiku`/`sonnet`/`opus` tier names name no model
here either (`docs/platform-matrix.md`, "Already measured" — the models measured in the
Ledger are differently named). Where the Claude Code build pins a cheap tier for discovery
and a strong one for synthesis, pick the model at invocation instead: `--model <name>`
(`docs/platform-matrix.md`, Item 8; Ledger #10). If you fan a sub-pass out through a
headless `agy -p` run, judge it by the **content of its `response` field, never by exit
code or `status`** — a run whose tool call was auto-denied still returns exit 0 and
`"status":"SUCCESS"` with an empty response (`docs/platform-matrix.md`, Item 8).
...
<!-- REPLACE-SECTION: ## Phase 2 — Run the expedition -->
